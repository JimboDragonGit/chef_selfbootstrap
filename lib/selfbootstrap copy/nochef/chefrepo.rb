  ...
  module SelfBootstrap
    module NoChef
      module ChefRepoHelpers
        ...
        def generate_policy(policyname)
          policyfile = ::File.join(workstation_policy_dir, policyname)
          chef(%w(generate policyfile) | [policyfile], as_system: true, live: true) unless ::File.exist?(policyfile + '.rb')
        end

        def write_kitchen_file
          worklog "Generate policy file #{workstation_resource[:environment]}"
          generate_policy workstation_resource[:environment]
